...
async function buildSignedNotification(doc) {
  // Ping whoever turns agents on: onboarding users/roles if set, else fall back
  // to the default notify role (mention:{} lets notifyDiscord apply that default).
  const roleIds = toIdList(ONBOARDING_ROLE_ID);
  const userIds = toIdList(ONBOARDING_USER_ID);
  const mention = roleIds.length || userIds.length ? { mentionRoleIds: roleIds, mentionUserIds: userIds } : {};
...
async function notifyDiscord(content, opts = {}) {
  ...
  const allowed = { parse: [] };
  let prefix = "";
  for (const id of userIds) prefix += `<@${id}> `;
  for (const id of roleIds) prefix += `<@&${id}> `;
