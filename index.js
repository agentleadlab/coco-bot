...
// Read a card's comments and decide whether setup is already done. Looks for a
// comment like "setup is done" / "set up complete" / "onboarding done", while
// ignoring negations ("setup not done", "setup pending").
async function getCardSetupStatus(cardId) {
  try {
    const actions = await trelloGet(`/cards/${cardId}/actions?filter=commentCard&limit=50`);
    for (const a of actions) {
      const t = (a?.data?.text || "").toLowerCase();
      if (!t) continue;
      if (/\bnot\b|pending|in progress|\bwip\b|to ?do|still|待/.test(t)) continue; // negations / not-yet
      const mentionsSetup = /set ?up|onboard/.test(t);
      const mentionsDone = /\b(done|complete|completed|finished|ready|good to go|live)\b|✅/.test(t);
      if (mentionsSetup && mentionsDone) return true;
    }
  } catch (err) {
    console.error(`trello: couldn't read comments for ${cardId}`, err.message ?? err);
  }
  return false;
}
...
function liveDayLabel(date) {
  return new Intl.DateTimeFormat("en-US", { timeZone: TRELLO_TZ, weekday: "short", month: "short", day: "numeric" }).format(date);
}
...
async function buildSignedNotification(doc) {
  // Ping whoever turns agents on: onboarding users/roles if set (placed at the
  // END for a cleaner look), else fall back to the default notify role.
  const roleIds = toIdList(ONBOARDING_ROLE_ID);
  const userIds = toIdList(ONBOARDING_USER_ID);
  const mention =
    roleIds.length || userIds.length
      ? { mentionRoleIds: roleIds, mentionUserIds: userIds, mentionPlacement: "suffix" }
      : {};

  const fallbackName = clientName(doc.name);
  const cleanFallback = () => ({
    content: [`🎉 **${fallbackName} signed!**`, `🔌 Set them up on the lead hub, then turn them on for their live date.`].join("\n"),
    mention,
  });
  if (!trelloEnabled) return cleanFallback();
  ...
  const setupDone = await getCardSetupStatus(card.id);

  const header = `🎉 **${agent} signed!**${setupDone ? " — already set up ✅" : ""}`;
  let line2;
  if (!live) {
    console.log(`trello: matched "${card.name}" but couldn't read a live date (setup ${setupDone ? "done" : "pending"})`);
    line2 = setupDone
      ? `📇 Set up already — check the card for the live date, then turn them on.`
      : `📇 On Trello, but I couldn't read a live date — check the card, then set up & turn them on.`;
  } else {
    ...
    if (liveYmd === todayYmd) {
      line2 = setupDone
        ? `🚀 Goes live **TODAY** — **turn them on now**.`
        : `🚀 Goes live **TODAY** — set them up and turn them on **now**.`;
    } else if (liveYmd > todayYmd) {
      line2 = setupDone
        ? `🗓️ Goes live **${label}** — just turn them on for that date.`
        : `🗓️ Goes live **${label}** — set them up and turn them on for that date.`;
    } else {
      line2 = setupDone
        ? `⚠️ Live date **${label}** already passed — turn them on ASAP.`
        : `⚠️ Live date **${label}** already passed — set up & turn on ASAP.`;
    }
  }
  return { content: [header, line2].join("\n"), mention };
}
...
// Accepts a single id, a comma/space-separated string of ids, or an array.
function toIdList(v) {
  if (!v) return [];
...
async function notifyDiscord(content, opts = {}) {
  const roleIds = toIdList(opts.mentionRoleIds ?? opts.mentionRoleId ?? process.env.DISCORD_NOTIFY_ROLE_ID);
  const userIds = toIdList(opts.mentionUserIds ?? opts.mentionUserId);
  const allowed = { parse: [] };
  if (userIds.length) allowed.users = userIds;
  if (roleIds.length) allowed.roles = roleIds;
  const mentionStr = [...userIds.map((id) => `<@${id}>`), ...roleIds.map((id) => `<@&${id}>`)].join(" ");
  let text = content;
  if (mentionStr) text = opts.mentionPlacement === "suffix" ? `${content}\n\n${mentionStr}` : `${mentionStr} ${content}`;
  const body = { content: text.slice(0, 2000), allowed_mentions: allowed };
...
async function handleWebhookEvents(events) {
  for (const evt of events) {
    ...
    if (doc.status === "document.completed") {
      ...
      await notifyDiscord(content, mention);
    } else {
...
function startServer() {
  const server = http.createServer(async (req, res) => {
    ...
    // Browser test: visit /test/signed?name=Anzac%20Keehan to fire the exact
    // "contract signed" flow (Trello live-date lookup + Discord ping) with no
    // real contract. Marked [TEST] in Discord so nobody mistakes it for real.
    if (req.method === "GET" && url.pathname === "/test/signed") {
      const name = (url.searchParams.get("name") || "Anzac Keehan").trim();
      res.writeHead(200, { "Content-Type": "text/plain" });
      res.end(`Triggering a TEST "signed" notification for "${name}" — check your Discord channel.`);
      (async () => {
        try {
          ...
          const fakeDoc = {
            ...
            grand_total: null,
            recipients: [{ first_name: parts[0], last_name: parts.slice(1).join(" "), has_completed: true }],
          };
          const { content, mention } = await buildSignedNotification(fakeDoc);
          await notifyDiscord("🧪 **[TEST]** " + content, mention);
          console.log(`test: posted manual signed notification for "${name}"`);
        } catch (err) {
          console.error("test: manual signed notification failed", err.message ?? err);
        }
      })();
      return;
    }
